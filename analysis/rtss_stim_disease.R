...
for (time in c("M0", "M3")) {
  ...
  for (stim in stims) {
  coeff <- glue("stimulation{stim}:casecase")
  ...
  cam_dis_rtss <- runGSEA_block_pid(mal_dis,
                     form_main_dis_case,
                     coef=coeff,
                     dup_cor_file = here(glue("output/dupcor/rtss_{time}_{stim}_disease_dupcor.rds")))

  suffix <- paste(stim, time, sep="_")
  write_csv(cam_dis_rtss, file.path(here("output/"), glue("disease_rtss_{suffix}.csv")))
  }
}

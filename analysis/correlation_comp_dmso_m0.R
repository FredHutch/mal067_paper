...
## RNAseq data, M0, DMSO, Comparator
m0_dmso_comp_cols <- meta %>%
...
m0_dmso_comp_pids <- meta %>%
  filter(col_id %in% m0_dmso_comp_cols) %>%
  pull(pid) %>%
  unique()

m0_expr <- exprAll[,m0_dmso_comp_cols]
...
antibody_cors <- m0_geneset_scores %>%
  ...
  summarize(n    = sum(!is.na(log10_mfi_no_dil)),
            cor  = cor.test(score, log10_mfi_no_dil,
                            method = "spearman")$estimate,
            pval = cor.test(score, log10_mfi_no_dil,
...
write_csv(iavi_cors, here("output/m0_dmso_comp_iavi_cors.csv"))
write_csv(antibody_cors, here("output/m0_dmso_comp_antibody_cors.csv"))
write_csv(cytokine_cors, here("output/m0_dmso_comp_cytokine_cors.csv"))

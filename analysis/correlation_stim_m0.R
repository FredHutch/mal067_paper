...
for (stim in stims) {
  ...
  m0_stim_geneset_scores[[stim]] <- stim_geneset_scores %>%
    left_join(meta %>% dplyr::select(col_id, pid),
              by = "col_id") %>%
    dplyr::select(col_id, pid, geneset, score)
}

dmso_geneset_scores <- m0_stim_geneset_scores$dmso %>%
  dplyr::rename(dmso_score = score) %>%
  dplyr::select(-col_id)

m0_stim_geneset_scores$dmso <- NULL

stim_dmso_geneset_scores <- NULL

for (stim in non_dmso_stims) {
  stim_geneset_scores <- m0_stim_geneset_scores[[stim]]
...
## correlations
  for (s in non_dmso_stims) {
    ...
    if (any(m3_pfs$stim == s)) {
      pfs_cors <- stim_geneset_scores %>%
        inner_join(m3_pfs, by = "pid") %>%
        filter(stim == s) %>%
        group_by(geneset, stim, tcell) %>%
        summarize(n    = sum(!is.na(PFS)),
                  cor  = cor.test(diff_score, PFS, method = "spearman")$estimate,
                  pval = cor.test(diff_score, PFS, method = "spearman")$p.value,
                  .groups = "drop")
      write_csv(pfs_cors, here(glue("output/m0_{s}_dmso_diff_rtss_pfs_cors.csv")))
    }

    write_csv(iavi_cors, here(glue("output/m0_{s}_dmso_diff_rtss_iavi_cors.csv")))
    write_csv(antibody_cors, here(glue("output/m0_{s}_dmso_diff_rtss_antibody_cors.csv")))
    write_csv(cytokine_cors, here(glue("output/m0_{s}_dmso_diff_rtss_cytokine_cors.csv")))
  }

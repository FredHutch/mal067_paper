  ...
  if (any(m3_pfs$stim == s)) {
    pfs_cors <- stim_geneset_scores %>%
      ...
      summarize(n    = sum(!is.na(PFS)),
                ...
                .groups = "drop")
    write_csv(pfs_cors, here(glue("output/m3_{s}_dmso_diff_rtss_pfs_cors.csv")))
  }

  write_csv(iavi_cors, here(glue("output/m3_{s}_dmso_diff_rtss_iavi_cors.csv")))
  write_csv(cevac_cors, here(glue("output/m3_{s}_dmso_diff_rtss_cevac_cors.csv")))
  write_csv(antibody_cors, here(glue("output/m3_{s}_dmso_diff_rtss_antibody_cors.csv")))
  write_csv(cytokine_cors, here(glue("output/m3_{s}_dmso_diff_rtss_cytokine_cors.csv")))
}

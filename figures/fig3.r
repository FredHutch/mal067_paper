...
dmso_case_control <- read_csv(here("output/disease_rtss_dmso_M3.csv"))
## AMA1 Cases vs Controls at M3
ama1_case_control <- read_csv(here("output/disease_rtss_ama1_M3.csv"))
## CSP Cases vs Controls at M3
csp_case_control <- read_csv(here("output/disease_rtss_csp_M3.csv"))
## HBS Cases vs Controls at M3
hbs_case_control <- read_csv(here("output/disease_rtss_hbs_M3.csv"))
...
plot_data <- bind_rows(
  .build_data(dmso_case_control, "DMSO", "Cases vs Controls"),
  .build_data(ama1_case_control, "AMA1", "Cases vs Controls"),
  .build_data(csp_case_control, "CSP", "Cases vs Controls"),
  .build_data(hbs_case_control, "HBsAg", "Cases vs Controls")
) %>%
...
wide_data <- plot_data %>%
  ...
  select(geneset, annotation, stimulation, log10fdr) %>%
  pivot_wider(id_cols = c("geneset", "annotation"),
              names_from = stimulation, values_from = log10fdr) %>%
  arrange(annotation)

write_csv(wide_data %>% select(geneset), here("output/sig_genesets_for_ind_analysis.csv"))

## just the heatmap matrix
hm_mat <- as.matrix(wide_data %>%
                      rename(HBS = HBsAg) %>%
                      select(DMSO, CSP, HBS, AMA1))
...
pdf(here("output/figures/Fig3.pdf"), width = 12, height = 9)
draw(fig3)
dev.off()
png(here("output/figures/Fig3.png"), width = 1000, height = 750)
draw(fig3)
dev.off()
